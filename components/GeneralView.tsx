...
import { Smile, Heart, Zap, Shield, AlertTriangle, MessageCircle, Clock, Book, Mic, Database, Palette, Type, Scissors, SmilePlus, Star, Calendar, Ruler, MapPin, Coffee, Utensils, Frown, Sun } from 'lucide-react';
    ...
    <div className="space-y-6 animate-fade-in pb-12">
      ...
      {activeSection === 'basic' && (
          <div className="space-y-6">
            {/* Character Profile Card (Visual Reference) */}
            <div className="bg-gradient-to-br from-emu-50 to-white dark:from-slate-800 dark:to-slate-900 p-0 rounded-[2rem] shadow-sm border border-emu-100 dark:border-slate-700 overflow-hidden relative group">
                {/* Decorative Elements mimicking the official art style */}
                <div className="absolute top-0 right-0 w-64 h-64 bg-emu-100/50 dark:bg-emu-900/20 rounded-full blur-3xl -mr-16 -mt-16 pointer-events-none"></div>
                <div className="absolute bottom-0 left-0 w-48 h-48 bg-yellow-100/50 dark:bg-yellow-900/20 rounded-full blur-3xl -ml-12 -mb-12 pointer-events-none"></div>
                
                <div className="p-8 relative z-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
                    {/* Left: Text Info */}
                    <div className="lg:col-span-2 space-y-6">
                        <div>
                            <div className="flex items-center gap-2 text-yellow-500 font-bold text-xs tracking-widest uppercase mb-1">
                                <Star size={14} fill="currentColor"/> Wonderlands x Showtime
                            </div>
                            <h1 className="text-4xl md:text-5xl font-black text-emu-600 dark:text-emu-400 font-sans tracking-tighter mb-2">
                                凤 笑梦
                                <span className="text-lg md:text-2xl font-normal text-emu-400 ml-4 opacity-70">Otori Emu</span>
                            </h1>
                            <p className="text-xs font-bold text-gray-400">CV. 木野 日菜</p>
                        </div>

                        <div className="bg-white/60 dark:bg-black/20 backdrop-blur-sm p-4 rounded-xl border border-emu-100 dark:border-slate-700 text-sm leading-relaxed text-gray-700 dark:text-gray-200">
                            性格天真烂漫，想到什么就会立刻付诸行动，因此常常把周围的人卷入其中。
                            为了让作为凤凰奇迹乐园的一大景点的舞台复活，正在寻找能一起站在舞台上的人。
                        </div>

                        <div className="grid grid-cols-2 gap-y-3 gap-x-8 text-sm">
                            <div className="flex items-center gap-3">
                                <div className="w-6 flex justify-center text-emu-400"><Calendar size={16}/></div>
                                <div><span className="text-xs text-gray-400 block">生日</span><span className="font-bold text-gray-700 dark:text-gray-200">9月9日</span></div>
                            </div>
                            <div className="flex items-center gap-3">
                                <div className="w-6 flex justify-center text-emu-400"><Ruler size={16}/></div>
                                <div><span className="text-xs text-gray-400 block">身高</span><span className="font-bold text-gray-700 dark:text-gray-200">152cm</span></div>
                            </div>
                            <div className="flex items-center gap-3">
                                <div className="w-6 flex justify-center text-emu-400"><MapPin size={16}/></div>
                                <div><span className="text-xs text-gray-400 block">学校</span><span className="font-bold text-gray-700 dark:text-gray-200">宫益坂女子学园</span></div>
                            </div>
                            <div className="flex items-center gap-3">
                                <div className="w-6 flex justify-center text-emu-400"><Coffee size={16}/></div>
                                <div><span className="text-xs text-gray-400 block">兴趣</span><span className="font-bold text-gray-700 dark:text-gray-200">探索邻里</span></div>
                            </div>
                            <div className="flex items-center gap-3 col-span-2">
                                <div className="w-6 flex justify-center text-emu-400"><Zap size={16}/></div>
                                <div><span className="text-xs text-gray-400 block">特技</span><span className="font-bold text-gray-700 dark:text-gray-200">杂技、无论什么都能吃得很香</span></div>
                            </div>
                            <div className="flex items-center gap-3">
                                <div className="w-6 flex justify-center text-emu-400"><Utensils size={16}/></div>
                                <div><span className="text-xs text-gray-400 block">喜欢的食物</span><span className="font-bold text-gray-700 dark:text-gray-200">鲷鱼烧</span></div>
                            </div>
                            <div className="flex items-center gap-3">
                                <div className="w-6 flex justify-center text-emu-400"><Frown size={16}/></div>
                                <div><span className="text-xs text-gray-400 block">讨厌的食物</span><span className="font-bold text-gray-700 dark:text-gray-200">无</span></div>
                            </div>
                            <div className="flex items-center gap-3 col-span-2">
                                <div className="w-6 flex justify-center text-emu-400"><Sun size={16}/></div>
                                <div><span className="text-xs text-gray-400 block">不擅长</span><span className="font-bold text-gray-700 dark:text-gray-200">黄昏 (夕暮れ)</span></div>
                            </div>
                        </div>
                    </div>
                    
                    {/* Right: Placeholder for Image (User can visualize their uploaded image here) */}
                    <div className="hidden lg:flex items-center justify-center relative">
                         <div className="w-full h-full bg-emu-200/20 dark:bg-emu-900/20 rounded-2xl border-2 border-dashed border-emu-200 dark:border-emu-800 flex flex-col items-center justify-center text-emu-300 gap-2 min-h-[300px]">
                            <img src="Emu_icon.png" alt="Emu" className="w-32 h-32 object-contain rounded-full shadow-lg mb-4 opacity-80" onError={(e) => e.currentTarget.style.display = 'none'} />
                            <span className="text-sm font-bold">设定参考图</span>
                            <span className="text-xs text-center px-4">此处展示翻译后的数据<br/>供配置 Bot 时参考</span>
                         </div>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-sm border border-emu-100 dark:border-slate-700">
                ...
                <Smile className="text-emu-500" /> 基本信息配置
                </h2>
